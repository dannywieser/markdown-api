...
beforeEach(() => {
  ...
  asMock(expandPath).mockImplementation((path: string) => `...${path}`)
})
...
describe('bear database functions', () => {
  ...
  test('creates the directory for the backups', () => {
    ...
    expect(createDir).toHaveBeenCalledWith('...~/.root-dir')
  })
  ...
  test('backupBearDatabase calls backupPrune with correct arguments', () => {
    ...
    expect(backupPrune).toHaveBeenCalledWith('bear-backup-', '...~/.root-dir', 2)
  })
